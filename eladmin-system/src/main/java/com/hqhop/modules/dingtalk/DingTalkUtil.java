...
//        // 确认url与配置的应用首页地址一致
        ...
        try {
            ticket = DingTalkUtil.getJsapiTicket();
...
//        try {
//            OapiProcessinstanceGetResponse processInfo = getProcessInfo("d90a6207-9a3b-413b-84dc-634d47c1a5e9");
...
//        }
        ...
        try {
            ...
            ArrayList empsList = new ArrayList<String>();
            boolean flag = true;
            Long offset = 0L;
            while (flag) {

                OapiSmartworkHrmEmployeeQueryonjobResponse empsFromDingtalk = getEmpsFromDingtalk("2", offset, 20L);
                if (empsFromDingtalk.getErrcode() == 0) {
                    ...
                    result.getDataList().stream().forEach(System.out::println);
                    try {
    ...
    /**
     * 在职员工子状态筛选，其他状态无效。2，试用期；3，正式；5，待离职；-1，无状态
     *
     * @param statusList ""2,3"表示查询试用期和正式员工
     * @param offSet     分页游标，从0开始
     * @param size       分页大小，最大20
     * @return
     * @throws ApiException
     */
    private static OapiSmartworkHrmEmployeeQueryonjobResponse getEmpsFromDingtalk(String statusList, Long offSet, Long size) throws ApiException {
