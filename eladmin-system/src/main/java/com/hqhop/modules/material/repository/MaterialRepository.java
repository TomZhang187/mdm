...
public interface MaterialRepository extends JpaRepository<Material, Long>, JpaSpecificationExecutor {
    ...
    @Query(value = "select * from material m inner join material_type mt on m.type_id=mt.type_id where mt.type_id in (select mt.type_id from material_type mt where mt.pid=?1) order by create_time DESC limit 1",nativeQuery = true)
